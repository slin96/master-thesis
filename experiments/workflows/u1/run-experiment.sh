...
docker exec server-container sh /server/experiments/workflows/u1/server-run-exp.sh -m $MODEL -a $APPROACH > server-log.log &
docker exec node-container sh /node/experiments/workflows/u1/node-run-exp.sh -a $APPROACH > node-log.log

echo "eval results"
docker exec eval-container sh /eval/experiments/workflows/u1/eval-run-exp.sh -a $APPROACH > eval-log.log
